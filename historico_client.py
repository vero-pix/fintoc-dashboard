...
class HistoricoClient:
    ...
    def guardar_saldos(self, total_clp, total_usd, total_eur, fondos_mutuos, por_cobrar, por_pagar_total):
        fecha = datetime.now().strftime('%Y-%m-%d %H:%M')
        row = [fecha, total_clp, total_usd, total_eur, fondos_mutuos, por_cobrar, por_pagar_total]
    ...
    def obtener_saldo_anterior(self):
        try:
            ...
            # Obtener última fila (saldo anterior)
            ultima = all_values[-1]
            return {
                'fecha': ultima[0],
                ...
                'por_pagar_total': float(ultima[6]) if ultima[6] else 0,
            }
...
if __name__ == "__main__":
    ...
    # Test guardar
    client.guardar_saldos(160000000, 83000, 80, 384000000, 416000000, 84000000)
    # Test leer
